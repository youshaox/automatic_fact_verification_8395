...
TASKNAME = '2_Pylucene-based-sentence'
...
def get_matched_by_lucene(directory, search_target, _dict, wiki_doc_dict, _type, K):
    ...
    with open(INTERMEDIATE_DATAPATH + 'pylucene_' + search_target  + str(K) + _type + '.pkl', 'wb') as fp:
...
@click.option('--k', default='25', type=int, help='number of top K')
def main(firsttime, dataset_type, k):
    ...
    try:
        ...
        matched_dict_sentence = get_matched_by_lucene(directory_sentence, search_target_sentence, data_dict, wiki_doc_dict, dataset_type, k)
